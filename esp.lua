...
local function showTip(text)
    TipLabel.Text = text
    local sz = TextService:GetTextSize(text, 11, Enum.Font.GothamSemibold, Vector2.new(600, 30))
...
local function hideTip()
    ...
    task.delay(0.2, function()
        if not tipVisible then TooltipFrame.Visible = false end
    end)
end
...
VersionLabel.Size                   = UDim2.new(0, 90, 0, 14)
VersionLabel.Position               = UDim2.new(0, 14, 0, 44)
VersionLabel.BackgroundTransparency = 1
VersionLabel.Text                   = "v3.2 - secure"
...
local function makeHBtn(xOff, bg, txt, txtCol)
    ...
    b.MouseEnter:Connect(function()
        tw(b, {BackgroundColor3 = bg:Lerp(Color3.fromRGB(255,255,255), 0.15)}, 0.15)
    end)
...
local CloseBtn = makeHBtn(-32, Color3.fromRGB(150,25,25), "X", Color3.fromRGB(255,255,255))
...
Lbl(C1,"Клавиши управления - в разделе ГОРЯЧИЕ КЛАВИШИ",14,32,240,14,9,Color3.fromRGB(50,50,96),false)

local TrTrack,TrKnob,ToggleHit = MkToggle(C1, W-88, 30)
hookTip(ToggleHit,"ToggleHit")

local C2 = Card(36,3)
local CntLbl = Lbl(C2,"Подсвечено: 0 игроков",14,0,180,36,12,Color3.fromRGB(100,100,155),false)
...
for _,b in ipairs({PrevBtn,NextBtn}) do
    ...
    b.MouseLeave:Connect(function() tw(b,{BackgroundColor3=Color3.fromRGB(22,22,40)},0.14) end)
end
...
local SliderBg,SliderFill,SliderKnob = MkSlider(FC,14,40,SW,Color3.fromRGB(70,40,200), 1-State.fillTrans)
...
local OutlineLbl = Lbl(OC,"Обводка: "..math.floor((1-State.outlineTrans)*100).."%",
    14,10,200,18,11,Color3.fromRGB(148,140,205),false)
local OSliderBg,OSliderFill,OSliderKnob = MkSlider(OC,14,40,SW,Color3.fromRGB(160,50,255), 1-State.outlineTrans)
...
local TagSizeLbl = Lbl(TC,"Размер текста: "..State.nameTagSize,14,154,200,18,11,Color3.fromRGB(148,140,205),false)
...
function createHL(player)
    ...
    local col = getColor()
    local rb  = isRainbow()
    local rc  = rb and getRainbow() or nil

    local hl = Instance.new("Highlight")
    hl.FillColor           = rc or col.fill
    hl.OutlineColor        = rc or col.outline
    hl.FillTransparency    = State.fillTrans
    hl.OutlineTransparency = State.outlineTrans
    hl.Adornee             = char
    hl.Parent              = ScreenGui
    ...
    local need = hrp and (State.showNames or State.showDistance or State.showHealth or State.showTool)
    if need then
...
-- ===========================================
--  SHOW / HIDE MENU  (fixed: full hide)
-- ===========================================
...
-- ===========================================
--  TAG UPDATE LOOP (optimized: ~0.15s, not every frame)
-- ===========================================
local tagAccum = 0
local TAG_INTERVAL = 0.15

RunService.RenderStepped:Connect(function(dt)
    local t = tick()

    -- animated gradients (lightweight)
    ...
    -- rainbow (only when active and selected)
    if State.enabled and isRainbow() then
        State.rainbowHue = (State.rainbowHue + dt*0.25) % 1
        local rc = getRainbow()
        for pl,hl in pairs(State.highlights) do
            hl.FillColor    = rc
            hl.OutlineColor = rc
    ...
    -- tags: update on interval, not every frame
    if State.enabled then
        tagAccum = tagAccum + dt
        if tagAccum >= TAG_INTERVAL then
            tagAccum = 0
    ...
    if math.abs(dx)+math.abs(dy) > 0.3 then
        local np = UDim2.new(
            cur.X.Scale + (tgt.X.Scale - cur.X.Scale)*LERP, cur.X.Offset + dx*LERP,
            cur.Y.Scale + (tgt.Y.Scale - cur.Y.Scale)*LERP, cur.Y.Offset + dy*LERP
        )
        Main.Position   = np
        Shadow.Position = UDim2.new(np.X.Scale, np.X.Offset, np.Y.Scale, np.Y.Offset - 6)
    end
end)
...
local function setEnabled(on)
    ...
    StatusDot.BackgroundColor3 = on and Color3.fromRGB(45,210,90) or Color3.fromRGB(200,45,45)
    StatusLbl.Text       = on and "ВКЛЮЧЕН" or "ВЫКЛЮЧЕН"
    StatusLbl.TextColor3 = on and Color3.fromRGB(45,210,90) or Color3.fromRGB(200,45,45)
    refreshAll()
end
...
UserInputService.InputBegan:Connect(function(inp, gp)
    if State.guiDestroyed then return end

    if State.bindingKey and inp.UserInputType == Enum.UserInputType.Keyboard then
        local kc = inp.KeyCode
        if kc == Enum.KeyCode.Escape then
            State.bindingKey = nil
            ESPBindBtn.Text = State.espKey.Name;  ESPBindBtn.TextColor3 = Color3.fromRGB(80,140,255)
            MenuBindBtn.Text = State.menuKey.Name; MenuBindBtn.TextColor3 = Color3.fromRGB(80,140,255)
            return
        end
        if State.bindingKey == "esp" then
            State.espKey = kc
            ESPBindBtn.Text = kc.Name; ESPBindBtn.TextColor3 = Color3.fromRGB(80,140,255)
        else
            State.menuKey = kc
            MenuBindBtn.Text = kc.Name; MenuBindBtn.TextColor3 = Color3.fromRGB(80,140,255)
        end
        State.bindingKey = nil
        return
    end
